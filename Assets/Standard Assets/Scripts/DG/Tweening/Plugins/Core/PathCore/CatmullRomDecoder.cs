...
namespace DG.Tweening.Plugins.Core.PathCore
{
	internal class CatmullRomDecoder : ABSPathDecoder
	{
		...
		internal override Vector3 GetPoint(float perc, Vector3[] wps, Path p, ControlPoint[] controlPoints)
		{
			...
			Vector3 a = (num3 != 0) ? wps[num3 - 1] : controlPoints[0].a;
			Vector3 a2 = wps[num3];
			Vector3 vector = wps[num3 + 1];
			Vector3 b = (num3 + 2 <= wps.Length - 1) ? wps[num3 + 2] : controlPoints[1].a;
		...
		internal void SetWaypointsLengths(Path p, int subdivisions)
		{
			...
			for (int i = 1; i < num; i++)
			{
				array2[0].a = ((i != 1) ? p.wps[i - 2] : p.controlPoints[0].a);
				array3[0] = p.wps[i - 1];
				array3[1] = p.wps[i];
				array2[1].a = ((i != num - 1) ? p.wps[i + 1] : p.controlPoints[1].a);
				float num2 = 0f;
