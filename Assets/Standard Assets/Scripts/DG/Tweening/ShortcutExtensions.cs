...
namespace DG.Tweening
{
	public static class ShortcutExtensions
	{
		...
		public static Sequence DOJump(this Rigidbody target, Vector3 endValue, float jumpPower, int numJumps, float duration, bool snapping = false)
		{
			...
			s.Append(DOTween.To(() => target.position, new DOSetter<Vector3>(target.MovePosition), new Vector3(endValue.x, 0f, 0f), duration).SetOptions(AxisConstraint.X, snapping).SetEase(Ease.Linear).OnUpdate(delegate
			{
				if (!offsetYSet)
				{
					offsetYSet = true;
		...
		public static Tweener DOShakeScale(this Transform target, float duration, float strength = 1f, int vibrato = 10, float randomness = 90f, bool fadeOut = true)
		{
			if (duration <= 0f)
			{
				UnityEngine.Debug.Log(Debugger.logPriority);
				if (Debugger.logPriority > 0)
				{
		...
		public static Sequence DOJump(this Transform target, Vector3 endValue, float jumpPower, int numJumps, float duration, bool snapping = false)
		{
			...
			{
				target.position = x;
			}, new Vector3(endValue.x, 0f, 0f), duration).SetOptions(AxisConstraint.X, snapping).SetEase(Ease.Linear).OnUpdate(delegate
			{
				if (!offsetYSet)
				{
					offsetYSet = true;
			...
			{
				target.position = x;
			}, new Vector3(0f, jumpPower, 0f), duration / (float)(numJumps * 2)).SetOptions(AxisConstraint.Y, snapping).SetEase(Ease.OutQuad).SetRelative<Tweener>().SetLoops(numJumps * 2, LoopType.Yoyo)).SetTarget(target).SetEase(DOTween.defaultEaseType);
			return s;
		}
		...
		public static Tweener DOBlendablePunchRotation(this Transform target, Vector3 punch, float duration, int vibrato = 10, float elasticity = 1f)
		{
			...
			return DOTween.Punch(() => to, delegate(Vector3 v)
			{
				...
				Quaternion lhs = Quaternion.Euler(v.x, v.y, v.z);
				Quaternion rhs = lhs * Quaternion.Inverse(rotation);
				to = v;
				target.rotation = target.rotation * Quaternion.Inverse(target.rotation) * rhs * target.rotation;
