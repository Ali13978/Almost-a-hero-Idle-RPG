...
{
	public class WeaponWoodRandomRanged : Weapon
	{
		...
		{
			return new WeaponWoodRandomRanged
			{
		...
		public override void UpdateActive(float dt)
		{
			...
			if ((this.attackTime > this.damageMoment || this.immediatelyTarget != null) && !this.hasThrown)
			{
				this.Throw();
			}
		...
		public void Throw()
		{
			...
			this.target = (this.immediatelyTarget ?? this.world.GetRandomAliveEnemy());
