...
{
	public class OLD_ArtifactEffectDamageBoss : ArtifactEffect
	{
		public override void Apply(UniversalTotalBonus totBonus)
		{
			totBonus.damageBossFactor -= this.amount;
		}

		public override ArtifactEffect GetCopy()
		{
			return new OLD_ArtifactEffectDamageBoss
			{
				amount = this.amount
			};
		}
		...
		public override double GetAmountAllowed(List<Artifact> otherArtifacts)
		{
			...
			foreach (Artifact artifact in otherArtifacts)
			{
				...
				{
					if (artifactEffect is OLD_ArtifactEffectDamageBoss)
					{
						num += ((OLD_ArtifactEffectDamageBoss)artifactEffect).amount;
					}
				}
			}
			return 0.9 - num;
		...
		{
			return OLD_ArtifactEffectDamageBoss.GetAmountString(-this.amount);
		}
		...
		{
			return OLD_ArtifactEffectDamageBoss.effectCategory;
		}
		...
		{
			return OLD_ArtifactEffectDamageBoss.GetEffectType();
		}
