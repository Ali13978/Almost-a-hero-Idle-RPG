...
{
	public class WeaponWood : Weapon
	{
		public override void Init(Hero by, World world)
		{
			...
			this.isActive = false;
			this.attackTime = 0f;
			this.waitTime = ((!this.doNotWaitForFirstAttack) ? 0f : this.durWait);
			this.hasDamaged = false;
		}

		public override Weapon Clone()
		{
			return new WeaponWood
			{
		...
		public void SetTiming(float durAttack, float damageMoment, float durAdditionalWait, float firstAttackTimeOffset = 0f)
		{
			this.durAttack = durAttack;
			this.durWait = durAttack + durAdditionalWait;
			this.damageMoment = damageMoment;
			this.firstAttackTimeOffset = firstAttackTimeOffset;
		}

		public void DoNotWaitForNextAttack()
		{
			this.doNotWaitForFirstAttack = true;
		}
		...
		private void Damage()
		{
			...
			UnitHealthy unitHealthy = this.immediatelyTarget ?? this.target;
		...
		private void Activate()
		{
			this.isActive = true;
			this.waitTime = 0f;
			this.attackTime = this.firstAttackTimeOffset;
			this.firstAttackTimeOffset = 0f;
			this.atSound = 0;
		}
		...
		private bool doNotWaitForFirstAttack;

		private float firstAttackTimeOffset;
	}
}
