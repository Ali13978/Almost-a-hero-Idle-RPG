...
{
	public class OLD_ArtifactEffectCritFactorHero : ArtifactEffect
	{
		public override void Apply(UniversalTotalBonus totBonus)
		{
			totBonus.critFactorHeroAdd += this.amount;
		}

		public override ArtifactEffect GetCopy()
		{
			return new OLD_ArtifactEffectCritFactorHero
			{
				amount = this.amount
			};
		}
		...
		public override double GetAmountAllowed(List<Artifact> otherArtifacts)
		{
			...
			foreach (Artifact artifact in otherArtifacts)
			{
				...
				{
					if (artifactEffect is OLD_ArtifactEffectCritFactorHero)
					{
						num += ((OLD_ArtifactEffectCritFactorHero)artifactEffect).amount;
					}
				}
			}
			return 6.0 - num;
		...
		{
			return OLD_ArtifactEffectCritFactorHero.GetAmountString(this.amount);
		}
		...
		{
			return OLD_ArtifactEffectCritFactorHero.effectCategory;
		}
		...
		{
			return OLD_ArtifactEffectCritFactorHero.GetEffectType();
		}
