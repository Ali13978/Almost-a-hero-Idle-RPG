...
{
	public class TrinketEffectDamageHigh : TrinketEffect
	{
		protected override void InitBuffData(ref List<BuffData> buffDataList)
		{
			...
			buffDataDamageTE.damageAdd = 0.32 + 0.08 * (double)this.level;
		...
		public override string GetDesc(bool withUpgrade, int lev = -1)
		{
			...
			string text = base.csg(GameMath.GetPercentString(0.32 + (double)lev * 0.08, false));
			if (withUpgrade)
			{
				text += base.csg(" (+" + GameMath.GetPercentString(0.08, false) + ")");
			}
		...
		{
			string percentString = GameMath.GetPercentString(0.32, false);
		...
		public const double DMG_BASE = 0.32;

		public const double DMG_LEVEL = 0.08;
	}
}
