...
{
	public class OLD_ArtifactEffectDamageHeroSkill : ArtifactEffect
	{
		public override void Apply(UniversalTotalBonus totBonus)
		{
			totBonus.damageHeroSkillFactor += this.amount;
		}

		public override ArtifactEffect GetCopy()
		{
			return new OLD_ArtifactEffectDamageHeroSkill
			{
				amount = this.amount
			};
		}
		...
		public override double GetAmountAllowed(List<Artifact> otherArtifacts)
		{
			...
			foreach (Artifact artifact in otherArtifacts)
			{
				...
				{
					if (artifactEffect is OLD_ArtifactEffectDamageHeroSkill)
					{
						num += ((OLD_ArtifactEffectDamageHeroSkill)artifactEffect).amount;
					}
				}
			}
			return 3.0 - num;
		...
		{
			return OLD_ArtifactEffectDamageHeroSkill.GetAmountString(this.amount);
		}
		...
		{
			return OLD_ArtifactEffectDamageHeroSkill.effectCategory;
		}
		...
		{
			return OLD_ArtifactEffectDamageHeroSkill.GetEffectType();
		}
