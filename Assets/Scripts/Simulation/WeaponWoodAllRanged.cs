...
{
	public class WeaponWoodAllRanged : Weapon
	{
		...
		{
			return new WeaponWoodAllRanged
			{
		...
		{
			this.hasThrown = true;
			this.target = (this.immediatelyTarget ?? this.target);
			...
			foreach (UnitHealthy unitHealthy in this.by.GetOpponents())
			{
				if (unitHealthy.IsAlive())
				{
					Projectile projectile = new Projectile(this.by, this.projectileType, this.targetType, unitHealthy, this.durFly, this.projectilePath);
					projectile.damage = damage2.GetCopy();
					...
					this.world.OnPreAttack(this.by, projectile.damage, projectile);
